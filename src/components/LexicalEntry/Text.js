...
const TextEntityContent = ({
  ...
  switch (mode) {
    case "edit":
      return (
        <div className="lingvo-input-buttons-group">
          {!(is_being_updated || edit) && <span className="lingvo-input-buttons-group__name">{content}</span>}

          {(is_being_updated || edit) && (
            <Input className="lingvo-input-action" onChange={event => setContent(event.target.value)} value={content} />
          )}
    ...
    case "view":
      return <span className="lingvo-entry-content">{entity.content}</span>;
    case "contributions":
...
const Edit = ({ onSave, onCancel, is_being_created }) => {
