...
class MapAreas extends PureComponent {
  ...
  componentDidMount() {
    ...
    const {
      data: { dictionaries: dict, loading }
    } = this.props;

    this.initAreas(dict, loading)
  }
  ...
  initAreas(allDictionary, loading) {
    if (!loading) {
      ...
      this.test = resultsCount.map((searches, dictionary) => {
      ...
      const pointsInPixel = this.test.map(point => this.latLngToLayerPoint(point.coords));
      console.log(pointsInPixel)
      this.updateMap(pointsInPixel, [], true);
    }
  ...
  updateMap(points, areas, areasMode, ) {
